...
UPDATE_id="UPDATE wyniki SET id=DEFAULT"
ALTAR_id="ALTER SEQUENCE wyniki_id_seq RESTART WITH 1"

def ALTAR_SEQ(connection):
    with get_cursor(connection) as cursor:
        cursor.execute(ALTAR_id,)



def update_id(connection):
    with get_cursor(connection) as cursor:
...
def dodaj_wynik(connection, nazwa , nr_start ,dystans ,czas , data,rok, lokalizacja  ):
    with get_cursor(connection) as cursor:
        cursor.execute(INSERT_wynik, (nazwa , nr_start ,dystans ,czas , data,rok, lokalizacja ))

def update_wynik(connection,id:int, nazwa , nr_start ,dystans ,czas , data,rok, lokalizacja ):
    with get_cursor(connection) as cursor:
        cursor.execute(UPDATE_wyniki, (id,nazwa , nr_start ,dystans ,czas , data,rok, lokalizacja ))
