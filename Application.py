...
class App(tk.Tk):
  def __init__(self):
    ...
    self.frame0 = tk.Frame(self, bg='white', height=200, width=500)
    self.frame0.grid_propagate(0)
    self.frame01 = tk.LabelFrame(self.frame0, bg='#6F7FEB',  height=210, width=180)
    self.frame01.grid_propagate(0)
    self.frame1 = tk.LabelFrame(self, bg='#6F7FEB', text="Okno wynikowe", fg="white", height=250, width=500)
    self.frame1.grid_propagate(0)
    self.frame02 = tk.Frame(self.frame0, bg='#6F7FEB', height=220, width=400)
    ...
    sb = tk.Scrollbar(self.frame1, orient='vertical', command=self.wyniki_table.yview, width=15)
    sb.grid(row=0, column=1, sticky='NS')
    self.wyniki_table.config(yscrollcommand=sb.set)
    sb.config(command=self.wyniki_table.yview)

    self.wyniki_table['columns'] = ('id', 'nazwa', 'nr_start', 'dystans', 'czas', 'data', 'rok', 'lokalizacja')
    ...
    self.wyniki_table.column('nr_start', anchor='center', width=70)
    self.wyniki_table.column('dystans', anchor='center', width=50)
    self.wyniki_table.column('czas', anchor='center', width=50)
    self.wyniki_table.column('data', anchor='center', width=50)
    self.wyniki_table.column('rok', anchor='center', width=40)
    self.wyniki_table.column('lokalizacja', anchor='center', width=80)
    ...
    def window_all():
        ...
        for wynik in Wyniki.all():
            self.wyniki_table.insert(parent='', index='end', values=(wynik.id, wynik.nazwa,wynik.nr_start, wynik.dystans, wynik.czas,wynik.data,wynik.rok, wynik.lokalizacja))

    def window_filtr():
        l1 = tk.Label(self.frame02,bg='#6F7FEB')
        nazwa_label = tk.Label(self.frame02,text="Nazwa biegu: ",bg='#6F7FEB', fg="white")
        nazwa_entry=tk.Entry(self.frame02, width=20)
        nr_start_label = tk.Label(self.frame02,text="Nr startowy: ",bg='#6F7FEB', fg="white")
        nr_start_entry=tk.Entry(self.frame02, width=20)
        dystans_label = tk.Label(self.frame02,text="Dystans: ",bg='#6F7FEB', fg="white")
        ...
        data_label = tk.Label(self.frame02,text="Dataatę[yyyy-mm-dd]: ",bg='#6F7FEB', fg="white")
        data_entry = tk.Entry(self.frame02, width=20)
        rok_label = tk.Label(self.frame02,text="Rok[yyyy-mm-dd]: ",bg='#6F7FEB', fg="white")
        rok_entry = tk.Entry(self.frame02, width=20)
        lokalizacja_label=tk.Label(self.frame02,text="Dodaj lokalizacje biegu(miasto):",bg='#6F7FEB', fg="white")
        ...
        def click():
            if (len(nazwa_entry.get())!=0):
                ...
                for wynik in Wyniki.filtr_nazwa(nazwa):
                    self.wyniki_table.insert(parent='', index='end', values=(
                    wynik.id, wynik.nazwa, wynik.nr_start, wynik.dystans, wynik.czas, wynik.data, wynik.rok,
                    wynik.lokalizacja))
            else:
                pass
            ...
            if (len(lokalizacja_entry.get())!=0):
                ...
                pass
        one_button = tk.Button(self.frame02, text="Ok",bg='#9EAAF2',fg="white", command=click)
    ...
    def window_one():
        one_message=tk.Message(self.frame02,text="Wybierz id wyniku który chcesz wyświetlić",bg='#6F7FEB',fg='white', justify='center',aspect=500)
        one_entry=tk.Entry(self.frame02,width=5)
        label_frame=tk.LabelFrame(self.frame02, bg='#6F7FEB', text="Wynik", fg="white", height=220, width=200)
        label_frame.grid_propagate(0)
        nr_label=tk.Label(label_frame,text="Nr.startowy", bg='#6F7FEB',fg="white")
        dystans_label=tk.Label(label_frame,text="Dystans", bg='#6F7FEB',fg="white")
        ...
        lokalizacja_label=tk.Label(label_frame,text="Lokalizacja", bg='#6F7FEB',fg="white")
        nazwa_label=tk.Label(label_frame,text="Nazwa", bg='#6F7FEB',fg="white")

        nr_label_put=tk.Label(label_frame,text=" ", bg='#6F7FEB',fg="white")
        dystans_label_put=tk.Label(label_frame,text=" ", bg='#6F7FEB',fg="white")
        ...
        lokalizacja_label_put=tk.Label(label_frame,text=" ", bg='#6F7FEB',fg="white")
            ...
            while True:
                try:
                    ...
                    if numer not in range(len(Wyniki.all())):
                        messagebox.showinfo(" ", "Wynik o takim id nie znajduje się w bazie")
                        break
                    else:
                        ...
                        break
        one_button = tk.Button(self.frame02, text="Ok",width=5,bg='#9EAAF2',fg="white", command=click)
        one_button.grid(row=0, column=2)

    def window_add():
        l1 = tk.Label(self.frame02,bg='#6F7FEB')
        nazwa_label = tk.Label(self.frame02,text="Wprowadź nazwe biegu: ",bg='#6F7FEB', fg="white")
        nazwa_entry=tk.Entry(self.frame02, width=20)
        nr_start_label = tk.Label(self.frame02,text="Dodaj nr startowy: ",bg='#6F7FEB', fg="white")
        nr_start_entry=tk.Entry(self.frame02, width=20)
        dystans_label = tk.Label(self.frame02,text="Dodaj dystans: ",bg='#6F7FEB', fg="white")
        dystans_entry = tk.Entry(self.frame02, width=20)
        czas_label = tk.Label(self.frame02,text="Dodaj czas (min): ",bg='#6F7FEB', fg="white")
        czas_entry = tk.Entry(self.frame02, width=20)
        data_label = tk.Label(self.frame02,text="Dodaj datę[yyyy-mm-dd]: ",bg='#6F7FEB', fg="white")
        data_entry = tk.Entry(self.frame02, width=20)
        rok_label = tk.Label(self.frame02,text="Dodaj rok: ",bg='#6F7FEB', fg="white")
        rok_entry = tk.Entry(self.frame02, width=20)
        lokalizacja_label=tk.Label(self.frame02,text="Dodaj lokalizacje biegu(miasto):",bg='#6F7FEB', fg="white")
        lokalizacja_entry = tk.Entry(self.frame02, width=20)
        l1.grid(row=0, column=0)
            ...
            while True:
                ...
                if len(data_entry.get())==0:
                    ...
                    try:
                        ...
                        messagebox.showinfo(" ", "Nieprawidłowa forma daty bądź wartość. Użyj formatu rok-miesiąc-dzień")
                ...
                app_dodaj_wynik(nazwa,nr_start,dystans,czas,data,rok,lokalizacja)
                tk.messagebox.showinfo(" ",f" Obiekt {nazwa} został dodany do bazy")
                break
        one_button = tk.Button(self.frame02, text="Ok",bg='#9EAAF2',fg="white", command=click)
        one_button.grid(row=11, column=1)
    ...
    def window_update():
        id_label = tk.Label(self.frame02,text="Wprowadź id wyniky do aktualizacji: ",bg='#6F7FEB', fg="white")
        nazwa_label = tk.Label(self.frame02,text="Wprowadź nazwe biegu: ",bg='#6F7FEB', fg="white")
        id_entry = tk.Entry(self.frame02, width=15)
        nazwa_entry=tk.Entry(self.frame02, width=15)
        nr_start_label = tk.Label(self.frame02,text="Wprowadź  nr startowy: ",bg='#6F7FEB', fg="white")
        nr_start_entry=tk.Entry(self.frame02, width=15)
        dystans_label = tk.Label(self.frame02,text="Wprowadź dystans: ",bg='#6F7FEB', fg="white")
        dystans_entry = tk.Entry(self.frame02, width=15)
        czas_label = tk.Label(self.frame02,text="Wprowadź czas(min): ",bg='#6F7FEB', fg="white")
        czas_entry = tk.Entry(self.frame02, width=15)
        data_label = tk.Label(self.frame02,text="Wprowadź datę[yyyy-mm-dd]: ",bg='#6F7FEB', fg="white")
        data_entry = tk.Entry(self.frame02, width=15)
        rok_label = tk.Label(self.frame02,text="Wprowadź  rok: ",bg='#6F7FEB', fg="white")
        rok_entry = tk.Entry(self.frame02, width=15)
        lokalizacja_label=tk.Label(self.frame02,text="Wprowadź lokalizacje biegu:",bg='#6F7FEB', fg="white")
        ...
        def click():
            while True:
                ...
                app_update_wynik(nazwa,nr_start,dystans,czas,data,rok,lokalizacja,id)
                tk.messagebox.showinfo(" ",f" Obiekt {nazwa} został zaktualizowany")
                break
        one_button = tk.Button(self.frame02, text="Ok",bg='#9EAAF2',fg="white", command=click)
        one_button.grid(row=11, column=1)
    ...
    def window_delete():
        ...
        one_message = tk.Message(self.frame02, text="Wybierz id wyniku który chcesz usunąć",bg='#6F7FEB', fg="white" ,justify='center',aspect=500)
        one_entry = tk.Entry(self.frame02, width=5)
        l1.grid(row=0, column=0)
        ...
        def click():
            try:
                ...
                if numer not in range(len(Wyniki.all())):
                    ...
                    tk.messagebox.showinfo(" ",f" Obiekt {numer} został usunięty z bazy")
        one_button = tk.Button(self.frame02, text="Ok",bg='#9EAAF2',fg="white", command=click)
    ...
    self.Button1 =tk.Button(self.frame01, text="Dodaj nowy wynik",width=24,bg='#8895EB',fg="white",command=click1)
    self.Button2 = tk.Button(self.frame01, text="Aktualizacja wyniku", width=24, bg='#8895EB', fg="white", command=click2)
    self.Button3 = tk.Button(self.frame01, text="Usuń wynik",width=24,bg='#8895EB',fg="white",command=click3)
    self.Button4 = tk.Button(self.frame01, text="Wyświetl wyniki",width=24,bg='#8895EB',fg="white", command=click4)
    self.Button5 = tk.Button(self.frame01, text="Wyświetl konkretny wynik",width=24,bg='#8895EB',fg="white", command=click5)
    self.Button6 = tk.Button(self.frame01, text="Filtr",width=24,bg='#8895EB',fg="white", command=click6)
    self.Button7 = tk.Button(self.frame01, text="Koniec",width=24,bg='#8895EB',fg="white", command=click7)
